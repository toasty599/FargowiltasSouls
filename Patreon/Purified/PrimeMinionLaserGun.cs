            ...
            else
            {
                ...
                if (targetting)
                {
                    ...
                    if (++Projectile.localAI[0] > 22)
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer)
                        {
                            int p = FargoSoulsUtil.NewSummonProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, 10f * Projectile.DirectionTo(targetnpc.Center), ProjectileID.MiniRetinaLaser, Projectile.originalDamage, Projectile.knockBack, Projectile.owner);
