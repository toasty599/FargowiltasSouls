...
namespace FargowiltasSouls.Patreon.Gittle
{
    public class RoombaPetBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ModContent.ProjectileType<RoombaPetProj>(), 0, 0f, player.whoAmI, 0f, 0);
            }
        }
    }
}