    ...
    public class BabySilhouette : ModProjectile
    {
        ...
        public override void AI()
        {
                ...
                && Main.wallLight[Framing.GetTileSafely(Projectile.Center).WallType])
            {
                for (int i = 0; i < 5; i++)
                {
                    int d = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 27, 0f, 0f, 0, default, 1f);
                ...
                for (int i = 0; i < 5; i++)
                {
                    int d = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 54, 0f, 0f, 0, default, 2f);
                    Main.dust[d].noGravity = true;
