            ...
            if (Timer >= 4)
            {
                ...
                if (t != -1)
                {
                    ...
                    if (distance.Length() < 400f)
                    {
                        distance.Normalize();
                        distance *= 6f;
                        int p = Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), npc.Center, distance, ProjectileID.FlamesTrap, npc.damage / 4, 0f, Main.myPlayer);
    ...
    public class SolarSpearman : EModeNPCBehaviour
    {
        ...
        public override bool CheckDead(NPC npc)
        {
            ...
            if (t != -1 && Main.player[t].active && !Main.player[t].dead && Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                velocity.Normalize();
                velocity *= 14f;
                Projectile.NewProjectile(npc.GetSpawnSourceForNPCFromNPCAI(), npc.Center, velocity, ModContent.ProjectileType<DrakanianDaybreak>(), npc.damage / 4, 1f, Main.myPlayer);
            }
