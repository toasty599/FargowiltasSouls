            ...
            if (++Projectile.ai[0] > 2)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    for (int i = -1; i <= 1; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.InheritSource(Projectile), spawnPos, Vector2.UnitY * 7f, ProjectileID.StardustJellyfishSmall, Projectile.damage, 0f, Main.myPlayer, 210);
                    }
                }
