        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (socket == null || core == null || socket.ai[3] != core.whoAmI || core.ai[0] == 2f)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                    Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MoonLordSunBlast>(), 0, 0f, Projectile.owner);
            }
            else
            {
                ...
                {
                    //perpendicular
                    /*Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MoonLordSunBlast>(),
                    ...
                    for (int i = 0; i < max; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center + Main.rand.NextVector2Circular(Projectile.width / 2, Projectile.height / 2), Vector2.Zero, ModContent.ProjectileType<MoonLordSunBlast>(),
