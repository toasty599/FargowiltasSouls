            ...
            if (npc != null && npc.ai[0] != 2f)
            {
                ...
                if (Projectile.localAI[1] == 0)
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        for (int i = -1; i <= 1; i++)
                        {
                            ...
                            Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, vel, ModContent.ProjectileType<GlowLine>(), 0, 0f, Main.myPlayer, 14f, Projectile.identity);
                        }
                    }
                }
                ...
                else if (Projectile.localAI[1] > 180)
                {
                    if (Projectile.localAI[1] % 6 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        float ai1New = (Main.rand.NextBool()) ? 1 : -1; //randomize starting direction
                        Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, vel, ModContent.ProjectileType<HostileLightning>(), Projectile.damage, Projectile.knockBack, Projectile.owner, vel.ToRotation(), ai1New * 0.75f);
                    }
                }
