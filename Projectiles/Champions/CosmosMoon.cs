    ...
    public class CosmosMoon : ModProjectile
    {
        ...
        public override void Kill(int timeLeft) //vanilla explosion code echhhhhhhhhhh
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                int max = 10;
                for (int i = 0; i < max; i++)
                    Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center + Main.rand.NextVector2Circular(Projectile.width / 2, Projectile.height / 2), Vector2.Zero, ModContent.ProjectileType<MutantBoss.MutantBomb>(), 0, Projectile.knockBack, Projectile.owner);
                if (!FargoSoulsUtil.BossIsAlive(ref NPCs.EModeGlobalNPC.moonBoss, NPCID.MoonLordCore))
                    Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<Masomode.MoonLordMoonBlast>(), 0, Projectile.knockBack, Projectile.owner, -Vector2.UnitY.ToRotation(), 32);
            }
