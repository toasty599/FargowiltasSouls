            ...
            if (Projectile.localAI[0] == 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    for (int i = 0; i <= modifier * 3; i++)
                        current = FargoSoulsUtil.NewSummonProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity, ModContent.ProjectileType<DestroyerBody2>(), Projectile.originalDamage, Projectile.knockBack, Projectile.owner, Main.projectile[current].identity);
                    int previous = current;
                    current = FargoSoulsUtil.NewSummonProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity, ModContent.ProjectileType<DestroyerTail2>(), Projectile.originalDamage, Projectile.knockBack, Projectile.owner, Main.projectile[current].identity);
