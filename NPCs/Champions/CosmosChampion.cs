        ...
        public override void AI()
        {
            ...
            if (NPC.localAI[3] == 0) //just spawned
            {
                ...
                if (NPC.ai[1] == 0)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                        Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<CosmosVortex>(), NPC.damage / 4, 0f, Main.myPlayer);
                }
            ...
            switch ((int)NPC.ai[0])
            {
                    ...
                    if (NPC.ai[1] == 0)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            if (FargoSoulsWorld.EternityMode)
                                Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<CosmosRitual>(), NPC.damage / 4, 0f, Main.myPlayer, 0f, NPC.whoAmI);
                            ...
                            for (int i = 0; i < max; i++)
                            {
                                Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<CosmosMoon>(), NPC.damage * 2 / 7, 0f, Main.myPlayer, MathHelper.TwoPi / max * i + startRotation, NPC.whoAmI);
                            }
                ...
                case 0: //float near player, skip next attack and wait if not p2
                    ...
                    if (NPC.ai[1] == 0 && !(NPC.localAI[2] == 0 || !Main.expertMode))
                    {
                        if (NPC.ai[0] != 5 && Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<GlowRing>(), NPC.damage / 4, 0f, Main.myPlayer, NPC.whoAmI, -23);
                    }
                ...
                case 5: //meteor punch
                    ...
                    else
                    {
                        ...
                        if (++NPC.ai[3] > 4)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, 0.5f * Vector2.UnitX, ModContent.ProjectileType<CosmosBolt>(), NPC.damage / 4, 0f, Main.myPlayer);
                                Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, -0.5f * Vector2.UnitX, ModContent.ProjectileType<CosmosBolt>(), NPC.damage / 4, 0f, Main.myPlayer);
                            }
                        }
                    }
                ...
                case 7: //vortex
                    ...
                    if (NPC.ai[1] == 30)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), player.Center, Vector2.Zero, ModContent.ProjectileType<CosmosVortex>(), NPC.damage / 4, 0f, Main.myPlayer, 0f, ai1);
                ...
                case 9: //shen ray and balls torture
                    ...
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<GlowRing>(), NPC.damage / 4, 0f, Main.myPlayer, NPC.whoAmI, -20);
                    }
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                for (int j = -1; j <= 1; j += 2)
                                {
                                    for (int i = -max; i <= max; i++)
                                    {
                                        ...
                                        Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, speed / 2, ModContent.ProjectileType<CosmosSphere>(), NPC.damage / 4, 0f, Main.myPlayer, travelTime, individualTiming);
                                    }
                                }
                            }
                        }
                ...
                case 11: //reticle and nebula blazes
                    ...
                    {
                        Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), player.Center, Vector2.Zero, ModContent.ProjectileType<CosmosReticle>(), NPC.damage / 4, 0f, Main.myPlayer, NPC.whoAmI);
                    }
                        ...
                        else
                        {
                            ...
                            if (Math.Abs(NPC.Center.Y - NPC.localAI[1]) < 300) //make the midpoint better at hitting people
                            {
                                ...
                                if (Main.netMode != NetmodeID.MultiplayerClient)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, modifier * 0.5f * vel, ModContent.ProjectileType<CosmosBolt>(), NPC.damage / 4, 0f, Main.myPlayer);
                                    Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, modifier * 0.5f * NPC.DirectionFrom(target), ModContent.ProjectileType<CosmosBolt>(), NPC.damage / 4, 0f, Main.myPlayer);
                                }
                            }
                            else if (++NPC.ai[2] > 1)
                            {
                                ...
                                if (Main.netMode != NetmodeID.MultiplayerClient)
                                {
                                    Vector2 target = new Vector2(NPC.localAI[0], NPC.localAI[1]);
                                    int modifier = Math.Sign(player.Center.X - target.X) == NPC.direction ? 1 : -1;
                                    Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, 0.5f * NPC.DirectionFrom(target), ModContent.ProjectileType<CosmosBolt>(), NPC.damage / 4, 0f, Main.myPlayer);
                                }
                            }
                        ...
                        /*if (FargoSoulsWorld.MasochistMode && NPC.localAI[2] != 0 && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            for (int i = 0; i < iMax; i++)
                            {
                                ...
                                for (int j = -jMax; j <= jMax; j++)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), spawnPos, vel, ModContent.ProjectileType<CosmosInvader>(), NPC.damage / 4, 0f, Main.myPlayer, ai0);
                                }
                            }
                        }*/
                    }
       ...
       public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            spriteBatch.Draw(npcTex, NPC.Center - screenPos + new Vector2(0f, NPC.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(rectangle), NPC.GetAlpha(drawColor), NPC.rotation, origin2, NPC.scale, effects, 0);
            spriteBatch.Draw(npcGlow2, NPC.Center - screenPos + new Vector2(0f, NPC.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(rectangle), glowColor, NPC.rotation, origin2, NPC.scale, effects, 0);
